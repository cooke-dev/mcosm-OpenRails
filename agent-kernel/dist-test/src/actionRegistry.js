...
export function authorityModeSatisfies(actual, required) {
...
export class ActionRegistry {
    ...
    register(action) {
        if (!/^[a-z][a-z0-9_]{2,63}$/.test(action.actionType))
            throw new Error("invalid actionType");
        const existing = this.actions.get(action.actionType);
        if (existing && JSON.stringify(existing) !== JSON.stringify(action))
            throw new Error(`action ${action.actionType} already registered with different terms`);
